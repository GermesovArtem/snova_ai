...
# --- RESET MIDDLEWARE (Ensures navigation commands always work) ---
class StateResetMiddleware(BaseMiddleware[Message]):
    async def pre(self):
        ...
        nav_cmds = ["начать", "старт", "/start", "✨ создать", "🤖 модель", "💳 баланс", "📞 контакты"]
        if text in nav_cmds or payload.get("command") == "start" or payload.get("cmd") in ["create", "model", "balance", "contacts", "main"]:
...
async def balance_handler(message: Message):
    async with AsyncSessionLocal() as db:
        user, _ = await services.get_or_create_user(db, message.from_id, platform="vk")
    await safe_vk_send(message.from_id, clean_markdown(messages.MSG_BALANCE_MENU.format(balance=int(user.balance))))
...
async def generic_handler(message: Message, existing_images=None, existing_vk_atts=None):
    ...
    if image_urls and not prompt:
         ...
         msg = "✨ **Фото приняты!**\n\nТеперь напишите задание: что нужно сделать? 👇"
         await safe_vk_send(message.from_id, clean_markdown(msg), attachment=",".join(vk_attachment_strs))
         return
    ...
    confirm_text = messages.MSG_CONFIRMATION.format(header=messages.MSG_CONFIRM_HEADER_NEW if not image_urls else messages.MSG_CONFIRM_HEADER_EDIT, safe_prompt=prompt[:100], img_count_text=f"Фото: {len(image_urls)} шт.\n" if image_urls else "", human_name=human_model_name(user.model_preference), ratio="auto", fmt="png", cost=int(cost), balance=balance)
                ...
                if info.get("state") in ["success", "completed"]:
                    ...
                    async with httpx.AsyncClient() as client:
                        ...
                        if r.status_code == 200:
                            ...
                            return
                elif info.get("state") in ["failed", "error"]: raise Exception(info.get("error", "Generation failed"))
            raise Exception("Timeout")
        except Exception as e:
            logger.error(f"GEN ERROR: {e}")
            try: await services.refund_frozen_credits(db, user_id, cost)
            except: pass
            await safe_vk_send(vk_p_id, f"❌ Ошибка генерации: {str(e)}", keyboard=keyboards.build_reply_kb())
